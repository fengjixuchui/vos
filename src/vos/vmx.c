...
void setup_vmx_PML4E (vos_guest_t* guest, vos_uint64 guest_VA, vos_uint64 GPA)
{
  ...
  if (pdpt_gpa == nullptr)
  {
    ...
    pdpt_hpa = (vos_uint64*)ept_translation ((ept_PML4E_t*)guest->physical_address_translation_pointer, (vos_uint64)pdpt_gpa);
    __memset64 (pdpt_hpa, 0, 512);
    pml4[pml4Idx] = (vos_uint64)pdpt_gpa | 7;
  }
  else
  {
    pdpt_hpa = (vos_uint64*)ept_translation ((ept_PML4E_t*)guest->physical_address_translation_pointer, (vos_uint64)pdpt_gpa);
  ...
  pd_gpa = (vos_uint64*)(pdpt_hpa[pdpIdx] & 0xfffffffff000);
  if (pd_gpa == nullptr)
  {
    pd_gpa = guest_malloc (guest, VOS_PAGE_SIZE);
    pd_hpa = (vos_uint64*)ept_translation (guest->physical_address_translation_pointer, (vos_uint64)pd_gpa);
    __memset64 (pd_hpa, 0, 512);
    pdpt_hpa[pdpIdx] = (vos_uint64)pd_gpa | 7;
  }
  else
  {
    pd_hpa = (vos_uint64*)ept_translation (guest->physical_address_translation_pointer, (vos_uint64)pd_gpa);
  ...
  pt_gpa = (vos_uint64*)(pd_hpa[pdIdx] & 0xfffffffff000);
  if (pt_gpa == nullptr)
  {
    pt_gpa = guest_malloc (guest, VOS_PAGE_SIZE);
    pt_hpa = (vos_uint64*)ept_translation (guest->physical_address_translation_pointer, (vos_uint64)pt_gpa);
    __memset64 (pt_hpa, 0, 512);
    pd_hpa[pdIdx] = (vos_uint64)pt_gpa | 7;
  }
  else
  {
    pt_hpa = (vos_uint64*)ept_translation (guest->physical_address_translation_pointer, (vos_uint64)pt_gpa);
...
vos_uint make_vmx_gdt (vos_guest_t* guest)
{
  ...
  vos_uint64* descriptor = ept_translation (guest->physical_address_translation_pointer, gdtr.base);
  descriptor[0]          = 0;
